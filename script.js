...
const SUGGESTIONS = {
  ...
  "Skin Color": [
    ...
    "Shadow Purple", "Phantom Blue", "Neon Pink", "Moonlight Lavender", "Sunset Orange",
    "Galactic Cyan", "Holographic Chrome", "Spirit Glow", "Celestial Tone",
    "Abyssal Shadow", "Ether Mist", "Blazing Ember", "Radiant Quartz", "Azure Soul"
  ],
  Headwear: [
    // 👒 Classic / Everyday
    ...
    "Hero Band", "Spirit Hood", "Celestial Halo", "Techno Crown", "Mystic Cap", "Battle Headband",
    "Phantom Veil", "Arcane Laurel", "Star Guardian Tiara", "Wind Warrior Hood", "Spirit Ember Crown",
    "Crimson Ronin Hat", "Plasma Helm", "Monk Wrap", "Frost Guard Helm", "Dusk Knight Mask", "Vortex Turban",
    "Crystal Diadem", "Sunfire Cap", "Shadow Veil Hood", "Solar Tiara", "Spirit Crown", "Battle Ribbon",
    "Mythic Commander Visor", "Lunar Laurel", "Energy Halo", "Wanderer Hat", "Warrior Tiara", "Mecha Band",
    "Infernal Crest", "Silent Monk Band", "Dune Nomad Hood", "Spectral Wreath", "Dream Weaver Band"
  ],
  Mask: [
    // ☣️ Modern / Tactical
    ...
    "Bone Carved Mask", "Stained Mask", "Cursed Mask", "Shadow Mask",
    ...
    "Spirit Fox Mask", "Cursed Soul Mask", "Shadow Oni Mask", "Glowing Visor", "Phantom Veil", "Ghost Samurai Mask",
    "Spirit Glass", "Bone Shade Mask", "Cyber Soul Mask", "Mystic Breath Mask", "Silent Warrior Mask", "Spirit Bandage Mask",
    "Crimson Shard Mask", "Aura Plate", "Rune Veil Mask", "Iron Phantom Mask", "Soulweaver Mask", "Celestial Guard Visor",
    "Spirit Chain Mask", "Lunar Fox Mask", "Astral Shadow Mask", "Glitch Veil", "Wraith Bone Mask", "Storm Oni Mask",
    "Sacred Cloth Mask", "Eclipse Breath Mask", "Cinder Plate Mask", "Aurora Mirror Mask", "Vortex Shard Mask", "Ash Guardian Mask",
  ...
  Eyes: [
   // 😐 Expressions / Emotion
    ...
    "Mechanical Eye", "Cracked Lens", "Side Eye", "Bruised Eye",
  ...
  Mouth: [
    ...
    "Scratch Fangs", "Monster Jaw", "Vampire Bite", "Zombie Jaw",
...
/* ---------- Additional suggestions loader from CSVs ---------- */
const CATEGORY_STRIP = {
  Eyes: ["eyes", "eye", "iris", "pupil", "pupils", "gaze", "vision"],
  Mask: ["mask", "faceplate", "visor", "respirator", "breather", "faceguard"],
  Mouth: ["mouth", "jaw"]
};
function normalizeForCategory(category, value) {
  const v = String(value || "").toLowerCase().trim();
  if (!v) return v;
  const strips = CATEGORY_STRIP[category] || [];
  let s = " " + v + " ";
  if (strips.length) {
    const re = new RegExp("\\b(" + strips.map(x => x.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|") + ")\\b", "gi");
    s = s.replace(re, " ");
  }
  s = s.replace(/[^a-z0-9]+/g, ""); // squash to alnum
  return s;
}
...
function addSuggestionsFromRows(rows) {
  if (!Array.isArray(rows) || !rows.length) return;
  const CATS = ["Skin Color","Headwear","Mask","Hair","Outfit","Back","Eyes","Mouth","Accessories"];
  // Build current normalized sets from existing SUGGESTIONS
  const existing = {};
  for (const cat of CATS) {
    const pool = getCombinedSuggestions(cat);
    const set = new Set(pool.map(v => normalizeForCategory(cat, v)));
    existing[cat] = set;
  }
  // Accumulate new items per category with local dedupe
  const pending = {};
  for (const cat of CATS) pending[cat] = [];
  for (const row of rows) {
    for (const cat of CATS) {
      const val = (row[cat] || "").trim();
      if (!val) continue;
      const key = normalizeForCategory(cat, val);
      if (!key) continue;
      if (existing[cat].has(key)) continue;
      // Skip if already queued in pending (based on normalized key)
      const already = pending[cat].some(v => normalizeForCategory(cat, v) === key);
      if (already) continue;
      pending[cat].push(val);
      existing[cat].add(key);
    }
  }
  // Merge into SUGGESTIONS (create category if missing)
  for (const cat of CATS) {
    if (!pending[cat].length) continue;
    if (!SUGGESTIONS[cat]) SUGGESTIONS[cat] = [];
    SUGGESTIONS[cat].push(...pending[cat]);
  }
}
async function loadAdditionalSuggestionsFromCSV(url) {
  try {
    const resp = await fetch(url, { cache: "no-store" });
    if (!resp.ok) return;
    const text = await resp.text();
    const rows = csvParseSimple(text);
    addSuggestionsFromRows(rows);
  } catch {
    // silently ignore (e.g., file:// fetch blocked)
  }
}
async function loadAdditionalSuggestions() {
  await loadAdditionalSuggestionsFromCSV("additional.csv");
  await loadAdditionalSuggestionsFromCSV("additional2.csv");
}
...
function renderDefaultTraits() {
  ...
  if (saved && Array.isArray(saved)) {
    ...
    initial = [...saved];
    for (const defaultTrait of DEFAULT_TRAITS) {
      // Ensure all traits from DEFAULT_TRAITS are present in the loaded set
      if (!savedKeys.has(defaultTrait.key)) {
        initial.push(defaultTrait);
      }
    }
  } else {
    initial = DEFAULT_TRAITS;
  }
...
function fillExample(includeAdditionalRandom = false) {
  ...
  const example = [
    ...
    { key: "Mask", value: "oni mask", include: true },
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initThemeControls();
  updateAllThemeAffectedUI();
  // Load additional trait suggestions from CSVs and refresh UI once loaded
  loadAdditionalSuggestions().then(() => {
    updateAllThemeAffectedUI();
  }).catch(() => {});
